...
class LargeEncoder(nn.Module):
    ...
    large_fc_group_input = 32
    large_fc_group_output = 1
    large_fc_groups = (latent_dim // large_fc_group_input)
    large_embedding_dim = large_fc_groups * large_fc_group_output
    embedding_dim = large_embedding_dim

    def __init__(self):
        super().__init__()

        self.encoder_conv = nn.Sequential(
            nn.Conv2d(1, 2, kernel_size=(1, 5), stride=(1, 2), padding=(0, 2)),
            nn.BatchNorm2d(2),
            nn.ELU(inplace=True),
            nn.Conv2d(2, 4, kernel_size=(5, 1), stride=(2, 1), padding=(2, 0)),
            nn.BatchNorm2d(4),
            nn.ELU(inplace=True),
            ...
            nn.BatchNorm2d(512),
            nn.ELU(inplace=True),
            nn.Conv2d(512, 1024, kernel_size=(3, 1), stride=(2, 1), padding=(1, 0)),
            nn.BatchNorm2d(1024),
            nn.ELU(inplace=True),
            nn.Conv2d(1024, self.latent_dim // 4, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)),
        ...
        self.encoder_large_div_encode = nn.ModuleList([
            nn.Sequential(
                nn.Linear(self.large_fc_group_input, 16),
                nn.BatchNorm1d(16),
                nn.ELU(inplace=True),
                nn.Linear(16, self.large_fc_group_output),
                nn.Sigmoid(),
